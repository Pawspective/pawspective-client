...
void NetworkClient::sendRequest(
    ...
    connect(
        ...
        this,
        [this, method, endpoint = request.url(), &data, reply, onSuccess = std::move(onSuccess), onError = std::move(onError)]() {
...
void NetworkClient::retryPendingRequests() {
    ...
    for (auto &request : pendingRequests) {
        sendRequest(request.method, request.endpoint, request.data, std::move(request.onSuccess), std::move(request.onError));
    }
}
